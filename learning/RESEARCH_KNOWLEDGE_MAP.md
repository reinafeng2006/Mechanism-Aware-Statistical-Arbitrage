...
**G2-05:** **APPROVED / FROZEN — 2026-09-04.** Trigger, Mechanism Discriminator, Sequential-Updating and Rejection/Rival Evidence are information roles rather than fixed variable types. `variable identity ≠ signal role`; each use is bound to a construct, hypothesis, decision time and PIT availability. Slow prior/context, medium relationship state, fast abnormality/market evidence and fast sequential updates remain distinct. Contemporaneous input, genuinely new sequential information and outcome-only validation remain separated. Independent Production Feasibility distinguishes research usefulness from production suitability using `EASY / CORE-CANDIDATE`, `MODERATE`, `HARD / OPTIONAL` and `UNAVAILABLE / RESEARCH-ONLY`; harder variables require role-relevant incremental value. No specific variable is rated and no activation approach, factor, formula, weight, update rule or model is selected.

**G2-06:** **APPROVED / FROZEN — 2026-09-07.** M1 under-response is insufficient peer movement in the expected signed direction relative to the current valid PIT relationship. Positive orientation means insufficient movement; overshoot has the opposite orientation; near-zero or too-uncertain direction cannot mechanically create M1 and may remain U. Expected response, observed response, signed gap, response uncertainty and under-response evidence remain distinct. UR0, UR1 and UR2 are competing **ILLUSTRATIVE / UNAUTHORIZED** candidates. UR2 separates normal-response, gap-measurement and additional discriminator/rival information sets to prevent double conditioning. UR0's transformation may be easy while end-to-end feasibility inherits Expected Signed Response. A gap identifies neither M1, probability nor future catch-up. No estimator, clock, window, threshold or model is selected.

**G2-07:** **APPROVED / FROZEN — 2026-09-07.** M2 separates expected/observed source response, oriented excess gap, uncertainty, pressure source, liquidity/flow state, contamination audit, rivals and downstream evidence. MP0–MP3 are competing **ILLUSTRATIVE / UNAUTHORIZED** candidates and form a complexity/information-requirement ladder—not an evidence-strength, identification-quality or model-maturity ranking. Every proxy requires a Contamination Record covering mechanical return content, endogeneity, overlapping inputs/windows, leakage, motive ambiguity and identification limits, plus seven-time PIT lineage. Excess movement, volume, turnover, flow, estimated transitory movement and later reversal do not identify M2. M1 and M2 are competing interpretations, not arithmetic negatives. No candidate, proxy, estimator, clock, frequency, window, threshold or model is selected.

**G2-08:** **APPROVED / FROZEN — 2026-09-07.** M0 requires positive rejection evidence. Pair Rejection, Event/Mechanism Rejection and Decision Rejection/Abstention remain distinct. Seven components separate relationship break, fundamental event, structural linkage, persistence, data quality, conflict and downstream M0 evidence. R0–R3 are **ILLUSTRATIVE / UNAUTHORIZED** candidates and are not combined into a score. R3 normally supports U rather than M0. `No Trade ≠ M0`, `U ≠ M0`, U does not default to M0, and future-discovered breaks or persistence cannot leak into the original decision time. No classifier, threshold, event/NLP model or trade rule is selected.

**G2-09:** **APPROVED / FROZEN — 2026-09-07.** M3's economic hypothesis is preserved, but literature ancestry is weak and positive identification absent; event-time discriminator and production signal/model are blocked. Residual/unexplained abnormality, exclusion, opposite direction, apparent temporariness and future normalization cannot create M3. Live unexplained cases route to U. Later discovery research is permitted only with target/input separation, PIT reconstruction, development/held-out separation and a formal G1 reopening trigger before production reconsideration. Future evidence may upgrade M3 or reject it as unnecessary/non-distinct. No M3 measurement candidate is created.

**G2-10:** **APPROVED / FROZEN — 2026-09-07.** U is a legitimate dynamic epistemic state under current PIT information, separate from Abstain as an action and No Trade as a possible outcome. Five overlapping uncertainty sources and U0–U4 candidate representation families are preserved without selection. `Not yet known` is distinct from `currently non-identifiable`; no probability simplex, threshold, transition or decision model is chosen.
...
**Pre-G3 readiness conclusion:** **APPROVED — 2026-09-07.** The project is not ready for acquisition. Five linked blockers are consolidated into **G2-12 PROPOSED / AWAITING RESEARCHER APPROVAL**: versioned Candidate Measurement Requirement Envelopes, target-universe and broad clock frames, dependency/inheritance graph, and Production Data Tiers. G3A feasibility audit is proposed but inactive; G3B acquisition remains locked.
