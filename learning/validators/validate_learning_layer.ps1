...
foreach ($boundary in @('HYPOTHESIS PRESERVED · LITERATURE WEAK','POSITIVE ID ABSENT · PRODUCTION BLOCKED','unexplained live abnormality → U','epistemic state — not mechanism','future signed follower catch-up；validation only','future source reversal/normalization；validation only','opposite-direction movement ≠ automatically abnormal','signal concept ≠ candidate measurement ≠ frozen factor ≠ model output ≠ trade rule')) { if ($dashboard -notmatch [regex]::Escape($boundary)) { throw "Missing mechanism/signal boundary: $boundary" } }
foreach ($uBoundary in @('U state ≠ Abstain action ≠ No Trade outcome','G2-10 FROZEN','U0 / U1 / U2 / U3 / U4 — ILLUSTRATIVE / UNAUTHORIZED')) { if ($dashboard -notmatch [regex]::Escape($uBoundary)) { throw "Missing G2-10 U boundary: $uBoundary" } }
