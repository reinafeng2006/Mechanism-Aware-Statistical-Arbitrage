...
The researcher [approved the scale architecture](decisions/S3_SCALE_ARCHITECTURE_APPROVAL.md) and [directly preregistered alpha=0.05, z0=Q^w_0.95(abs(z_ref))](decisions/S3_REFERENCE_TAIL_PROBABILITY_PREREGISTRATION.md). Numerical z0 remains unestimated, fallback inactive, occurrence/support selection of the primary threshold excluded. The researcher [approves the minimal capture/cost architecture](decisions/S3_MINIMAL_SCENARIO_CAPTURE_COST_ARCHITECTURE_APPROVAL.md): F/P/N/A/X, pooled prior-event P1, joint duration/exposure, one all-leg ledger and E1, with P3 inactive. Unresolved boundaries/support and all estimation stay gated. [H-C and enumerated execution/accounting choices](decisions/S3_HC_PROXY_HEDGE_EXECUTION_ACCOUNTING_APPROVAL.md) are approved: common proxy universe/rule, deterministic abort/unwind, E-A, intended K and exit precedence. H-B is benchmark only; H-A inactive. [Fund-share/M1/static-origin/T1 construction](decisions/S3_PROXY_CONSTRUCTION_APPROVAL.md) and strict missing-industry unavailability are approved. Actual funds, auxiliary exposure parameters and qualification values remain unselected. The [convergence target bridge](stages/G5/S3_CONVERGENCE_TARGET_BRIDGE_DESIGN_CHECKPOINT.md) compares only A and B; neither is approved and model z is unchanged. NEXT_ACTION remains NONE; no data access, estimation or implementation. Frozen V1 is unchanged.
...
| G4-03B3 Nested Fold Count, Origin Structure & Boundary Geometry | **APPROVED / FROZEN — OF4 ANNUAL OUTER FOLDS; SEMIANNUAL INNER ORIGINS / NO COMPUTATION** |
...
| G4-05B R2 Company-Characteristic-Conditioned Relationship Specification | **PARTIALLY APPROVED — R2-LIS + OLS PRIMARY + RIDGE SOLE CHALLENGER / R2-N DEFERRED / DATA AND EXECUTABLE BINDINGS PENDING** |
...
| G4-05C R3 Hierarchical / Partial-Pooling Specification | **GAUSSIAN EB RANDOM INTERCEPT/SLOPE + P0 APPROVED IN PRINCIPLE / EXECUTABLE ESTIMATOR UNRESOLVED / NO FITTING** |
| G4-05D R4 Dynamic / State-Space Specification | **STATIC INTERCEPT + RANDOM-WALK SLOPE APPROVED IN PRINCIPLE / VARIANCE-CLOCK CONTRACT UNRESOLVED / NO FITTING** |
| G4-05E R5 Long-Run / Cointegration Specification | **R5-EG-ECM REGISTERED / BLOCKED BY C04 + PRICE-CONSTRUCTION CONTRACT / VECM DEFERRED** |
| G4-05B1 P1 Fundamentals Field/PIT Audit | **DOCUMENTATION AUDIT COMPLETE / SCALE QUALIFIED WITH LIMITATIONS / ACCOUNTING CONSTRUCTS UNRESOLVED / NO ACQUISITION** |
| Five-Day Research + Trading V1 Mode | **ACTIVE — SCOPE CLOSED / PHASE 1 INNER DEVELOPMENT AUTHORIZED** |
| V1 Pre-Computation + Trading Protocol | **APPROVED / FROZEN — C04-A + MODEL-A + SCI-A + TRADE-A** |
| G4-04A6 Resolution / Predictive Validation | **SCI-A APPROVED / FROZEN — INNER DEVELOPMENT ONLY** |
| G4-05 V1 Executable Registry | **MODEL-A APPROVED / FROZEN — SEVEN TUPLES; R2 V2; R5 BLOCKED** |
| G5 Trading Simulation V1 Protocol | **TRADE-A APPROVED / FROZEN — EXIT-RESOLVE-10 / C04-A CONDITIONAL** |
| G5 Implementation Readiness | **PHASE 1 + 2015–2019 INNER AUTHORIZED / OF4 AND HELD-OUT LOCKED** |
