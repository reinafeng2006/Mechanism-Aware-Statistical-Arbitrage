...
|---|---|
| Current stage | `V1 PHASE 1 / DECISION CADENCE` |
| Stage status | `DC-A + EP-A SELECTED — PUBLICATION REQUIRED BEFORE RESUME` |
| Latest frozen decision | `V1-EXECUTABLE-SEMANTICS-A1 — APPROVED / FROZEN` |
| Current substage | publish daily decision-event and one-active-episode semantics |
...
| Computation authorization | denied until DC-A + EP-A publication; OF4 and final held-out denied |
| Current authorized action | `V1-DC-A-EP-A-PUBLICATION-V1` |
| Last validation state | A6-TG-A and its role guard published; no model fit or outcome computed |
| Latest local commit | `c1ee845` — A6-TG-A inner execution guard published |
| Remote state observed | `origin/main` at `c1ee845` |
| Researcher action required | `NO`; publish DC-A + EP-A then resume inner computation |
