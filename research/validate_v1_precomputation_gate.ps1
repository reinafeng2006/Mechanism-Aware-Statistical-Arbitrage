...
$preHeldOutEvidenceGate = (
    $action.action.action_id -eq 'NONE' -and
    $action.action.execution_state -eq 'PRE_HELD_OUT_V1_GATE_RESEARCHER_DECISION_REQUIRED' -and
    $action.action.dataset_access -eq 'DENIED_PENDING_PRE_HELD_OUT_RESEARCHER_DECISION' -and
    $action.action.held_out_access -eq 'SEALED_DENIED'
)
$h126Amendment = (
...
$finalHeldout = (
    ...
    $action.action.execution_state -in @('HELDOUT_RUNNER_PREPARATION_AUTHORIZED_ACCESS_NOT_YET_OPENED','HELDOUT_R4_RESUME_READY_AT_2025','HELDOUT_R4_CHECKPOINTED_RESUME_READY_AT_2025','HELDOUT_R4_FINAL_DAY_DEADLINE_MONITOR_ACTIVE','HELDOUT_R4_COMPUTATION_INCOMPLETE_REDUCED_SUPPORT_PLAN_ACTIVE') -and
