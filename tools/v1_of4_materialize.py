...
FINAL_MANIFEST = ROOT / "data/manifests/V1_OF4_RELATIONSHIP_STAGE.json"
...
def transform_sources(input_hash: str | None = None) -> dict[str, Path]:
    ...
    src = replace_required(src, 'LOWER, UPPER = "20130107", "20191231"', 'LOWER, UPPER = "20130107", "20231231"', 1)
    src = replace_required(src, 'V1-PHASE1-INNER-INPUT-1.0', 'V1-PO-C-OF4-INPUT-1.0')
    src = replace_required(src, 'IMMUTABLE DERIVED INNER INPUT — NO MODEL OUTPUT', 'IMMUTABLE DERIVED OF4 INPUT — NO MODEL OUTPUT')
    src = replace_required(src, '"of4_accessed": False', '"of4_accessed": True', 1)
    src = replace_required(src, 'OUT.relative_to(ROOT).as_posix()', 'str(OUT)', 1)
    paths["prepare"] = write_runner("prepare.py", src)
    if input_hash is None:
        return paths

    annual_double = 'periods=[(f"{y}",f"{y}0101",f"{y}1231") for y in range(2020,2024)]'
    annual_single = "periods=[(f'{y}',f'{y}0101',f'{y}1231') for y in range(2020,2024)]"
    ...
    src = replace_required(src, '2015-2019_INNER_ONLY', '2020-2023_OF4_ONLY')
    ...
    src = replace_required(src, "if dates[-1]>'20191231'", "if dates[-1]>'20231231'", 1)
...
def relationship_manifest(input_hash: str) -> None:
    ...
    for candidate in CANDIDATES:
        for year in YEARS:
            ...
            entries.append({"candidate": candidate, "partition": year, "artifact": str(payload), "rows": int(meta["rows"]), "sha256": meta["sha256"], "of4_accessed": True, "held_out_accessed": False})
    manifest = {"manifest_id": "V1-PO-C-OF4-RELATIONSHIP-OUTPUTS-1.0", "status": "IMMUTABLE_COMPLETE", "input_sha256": input_hash, "candidate_count": 7, "partition_count": 28, "partitions": entries, "date_role": "2020-2023_OF4_ONLY", "interpretation": "NONE_BEFORE_COMPLETE_MATERIALIZATION", "of4_accessed": True, "held_out_accessed": False}
...
def main() -> None:
    ...
    if args.stage in ("prepare", "all") and not INPUT.exists():
        ...
        with np.load(INPUT_TEMP, allow_pickle=False) as prepared:
            ...
            if dates[0] != "20130107" or dates[-1] != "20231229" or np.any(dates > "20231231"):
                raise RuntimeError("OF4 input temporal boundary validation failed")
